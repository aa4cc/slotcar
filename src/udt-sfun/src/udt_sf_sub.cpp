...
static void mdlCheckParameters(SimStruct *S)
{
    ...
    bool isValid = isPositiveRealDoubleParam(ssGetSFcnParam(S, HOST_PORT_P));
    if (!isValid)
    {
    ...
    isValid = isPositiveRealDoubleParam(ssGetSFcnParam(S, DATA_WIDTH_P));
    if (!isValid)
    {
        ssSetErrorStatus(S, "Data width parameter must be a positive scalar.");
        return;
...
static void mdlSetupRuntimeResources(SimStruct *S)
{
    ...
    port_p = ssGetSFcnParam(S, HOST_IP_P);
    ...
    sock = new UDTSOCKET(UDT::socket(hints.ai_family, hints.ai_socktype, hints.ai_protocol));

    // socket options
    bool block = false, reuse = false;
    if (UDT::ERROR == UDT::setsockopt(*sock, 0, UDT_RCVSYN, &block, sizeof(bool)) ||
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    if (UDT::ERROR == UDT::recvmsg(*udtSocket, (char*) uptr, usize)) {
